...
export class LoadedBinding {
  ...
  async doConnect(): Promise<void> {
    ...
    const [upstreamCompId, upstreamPortName] = binding.fromPort.split('.');
    const [downstreamCompId, _downstreamPortName] = binding.toPort.split('.');
    ...
    if (binding.filters && binding.filters.length > 0) {
      let currentSchema = upstreamNode
        .getOutputPort(upstreamPortName)
        ?.schema.get() || { type: 'any' };
      ...
      for (let i = 0; i < binding.filters.length; i++) {
        ...
        const filter = new Filter(manifest, filterDef, filterId);

        // Add filter node to graph
        this.parentGraph.addNode(filter.node);

        // Setup ports with current upstream type
        filter.node.setupPorts(currentSchema, filterDef);

        this.addFilter(filter);

        // Update type for next filter/downstream
        currentSchema = filter.getOutputSchema(currentSchema);
      }
    }
  ...
  disconnect() {
    ...
    this.destinationNode
      .getInputPort(this.destinationPortName)
